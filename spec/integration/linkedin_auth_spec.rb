...
describe "LinkedIn OAuth2" do
  ...
  def setup_linkedin_emails_stub(email:)
    if email
      body = { elements: [{ "handle~": { emailAddress: email } }] }
    else
      body = {}
    end
    stub_request(
      :get,
      "https://api.linkedin.com/v2/emailAddress?projection=(elements*(handle~))&q=members",
    ...
    stub_request(
      ...
      body:
        JSON.dump(
          ...
          profilePicture: {
            "displayImage~": {
              elements: [{ identifiers: [{ identifier: "https://linkedin.com/imarge-url.jpg" }] }],
            },
          },
        ),
      headers: {
  ...
  it "signs in the user whose email matches the email included in the API response from linkedin" do
    ...
    post "/auth/linkedin/callback", params: { state: session["omniauth.state"], code: temp_code }
    expect(response.status).to eq(302)
